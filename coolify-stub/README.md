...
talks to the coold broker over its HTTP-over-UDS lane.
...
Requires a broker socket at `$BROKER_SOCKET_PATH`. For local work, either run
the broker against a scratch path or `ssh -L` the Hetzner socket.
...
| `BROKER_SOCKET_PATH` | `/run/coolify/broker.sock`   | Coold broker UDS path                    |
...
the running broker, then run it with the env vars above. No runtime deps.
