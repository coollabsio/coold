//! Builder + broker lifecycle e2e. Provisions 2 Hetzner VMs
//! (A = central + builder, B = coold-only), runs `coolify init apply`,
//! then exercises every broker dispatch path, cancel, restart-adoption,
...
use e2e_tests::install::{local_coolify, unit_active, wg0_ip, InstallEnv};
use e2e_tests::{
...
fn builder_lifecycle() {
    ...
    step("1/9  coolify init apply (A=central+builder, B=coold-only)");
    local_coolify(
        &cfg.coolify_bin,
        &[
            "init",
    ...
    // 2. Wait for coold + broker to be active. The CLI returns once the
    //    systemd units are enabled, but the services may still be in
    //    `activating`; dispatch tests fail hard if broker isn't ready.
    step("2/9  wait systemd units active");
    assert!(
        wait_until(
            || unit_active(&cfg.ssh_key, &host_a, "broker"),
            Duration::from_secs(60),
        ),
        "broker never active on {host_a}"
    );
    ok(&format!("{host_a}: broker active"));
    assert!(
        wait_until(
    ...
    //    values the broker matches against.
    ...
    // 4-9. Cheap scenarios first so slow-repo flakes don't mask cheap
    //      coverage.
    step("4/9  scenario: pin_to_builder_host");
    scenario_pin_to_builder(&env);
    step("5/9  scenario: pin_to_coold_only returns 503");
    scenario_pin_to_coold_only_503(&env);
    step("6/9  scenario: unknown host_id returns 503");
    scenario_unknown_host_503(&env);
    step("7/9  scenario: load balance picks builder");
    scenario_load_balance(&env);
    step("8/9  scenario: cancel + artifact perms");
    scenario_cancel_and_artifact_perms(&env);
    step("9/9  scenario: coold restart adopts in-flight build");
